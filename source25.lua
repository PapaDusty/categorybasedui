...
local uipallet = {
    Main = Color3.fromRGB(20, 20, 20),  -- Changed to 20,20,20
...
function mainapi:BlurCheck()
    if self.ThreadFix then
        setthreadidentity(8)
        local blurEnabled = self.Blur and self.Blur.Enabled or false
        runService:SetRobloxGuiFocused((clickgui.Visible or guiService:GetErrorType() ~= Enum.ConnectionError.OK) and blurEnabled)
    end
end
...
function mainapi:CreateGUI()
    ...
    discordbutton.Image = 'rbxassetid://14368318994'
    discordbutton.Parent = window
    addTooltip(discordbutton, 'Copy Discord invite')
    ...
    panetitle.Name = 'Title'
    panetitle.Size = UDim2.new(1, -36, 0, 20)
    panetitle.Position = UDim2.fromOffset(math.abs(panetitle.Size.X.Offset), 11)
    panetitle.BackgroundTransparency = 1
    panetitle.Text = 'Settings'
    panetitle.TextXAlignment = Enum.TextXAlignment.Left
    panetitle.TextColor3 = uipallet.Text
    panetitle.TextSize = 13
    panetitle.FontFace = uipallet.Font
    panetitle.Parent = settingspane
    local paneclose = addCloseButton(settingspane)
    ...
    paneback.Image = 'rbxassetid://14368303894'
    paneback.ImageColor3 = color.Light(uipallet.Main, 0.37)
    paneback.Parent = settingspane
    ...
    function categoryapi:CreateSettingsPane(categorysettings)
        ...
        panetitle.Position = UDim2.fromOffset(math.abs(panetitle.Size.X.Offset), 11)
        panetitle.BackgroundTransparency = 1
        panetitle.Text = categorysettings.Name
        panetitle.TextXAlignment = Enum.TextXAlignment.Left
        panetitle.TextColor3 = uipallet.Text
        panetitle.TextSize = 13
        panetitle.FontFace = uipallet.Font
        panetitle.Parent = pane
        local paneclose = addCloseButton(pane)
        local paneback = Instance.new('ImageButton')
        paneback.Name = 'Back'
        paneback.Size = UDim2.fromOffset(16, 16)
        paneback.Position = UDim2.fromOffset(11, 13)
        ...
        paneback.Image = 'rbxassetid://14368303894'
        paneback.ImageColor3 = color.Light(uipallet.Main, 0.37)
        paneback.Parent = pane
        addCorner(pane)
        local panechildren = Instance.new('Frame')
        panechildren.Name = 'Children'
        panechildren.Size = UDim2.new(1, 0, 1, -57)
        panechildren.Position = UDim2.fromOffset(0, 41)
        panechildren.BackgroundColor3 = uipallet.Main
        ...
        paneback.MouseEnter:Connect(function()
            paneback.ImageColor3 = uipallet.Text
        end)
        paneback.MouseLeave:Connect(function()
            paneback.ImageColor3 = color.Light(uipallet.Main, 0.37)
        end)
        ...
        panewindowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
            ...
            pane.Size = UDim2.fromOffset(220, 45 + panewindowlist.AbsoluteContentSize.Y / scale.Scale)
        end)
    ...
    function categoryapi:CreateOverlayBar()
        ...
        overlayicon.Size = UDim2.fromOffset(14, 12)
        overlayicon.Position = UDim2.fromOffset(10, 13)
        overlayicon.BackgroundTransparency = 1
        overlayicon.Image = 'rbxassetid://14368339581'
        overlayicon.ImageColor3 = uipallet.Text
        overlayicon.Parent = overlaywindow
        local overlaytitle = Instance.new('TextLabel')
        overlaytitle.Name = 'Title'
        overlaytitle.Size = UDim2.new(1, -36, 0, 38)
        overlaytitle.Position = UDim2.fromOffset(36, 0)
        overlaytitle.BackgroundTransparency = 1
        overlaytitle.Text = 'Overlays'
        overlaytitle.TextXAlignment = Enum.TextXAlignment.Left
        overlaytitle.TextColor3 = uipallet.Text
        overlaytitle.TextSize = 15
        overlaytitle.FontFace = uipallet.Font
        overlaytitle.Parent = overlaywindow
        local overlayclose = addCloseButton(overlaywindow, 7)
        local overlaydivider = Instance.new('Frame')
        overlaydivider.Name = 'Divider'
        overlaydivider.Size = UDim2.new(1, 0, 0, 1)
        overlaydivider.Position = UDim2.fromOffset(0, 37)
        overlaydivider.BackgroundColor3 = color.Light(uipallet.Main, 0.02)
        overlaydivider.BorderSizePixel = 0
        overlaydivider.Parent = overlaywindow
        ...
        local togglewindowlist = Instance.new('UIListLayout')
        togglewindowlist.SortOrder = Enum.SortOrder.LayoutOrder
        togglewindowlist.HorizontalAlignment = Enum.HorizontalAlignment.Center
        togglewindowlist.Parent = childrentoggle
        ...
        button.MouseButton1Click:Connect(function()
            ...
            tween:Tween(overlaywindow, uipallet.Tween, {
                Position = UDim2.new(0, 0, 1, -(overlaywindow.Size.Y.Offset))
            })
        end)
        ...
        togglewindowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
            if mainapi.ThreadFix then
                setthreadidentity(8)
            end
            overlaywindow.Size = UDim2.fromOffset(220, math.min(37 + togglewindowlist.AbsoluteContentSize.Y / scale.Scale, 605))
    ...
    discordbutton.MouseButton1Click:Connect(function()
        ...
        mainapi:CreateNotification("Discord", "Invite link copied!", 2)
    end)
...
-- Create the GUI (this must run before accessing Categories.Main)
mainapi:CreateGUI()
