...
components.Targets = function(optionsettings, children, api)
    ...
    function optionapi:Load(tab)
        if self.Players and self.Players.Enabled ~= tab.Players then
            self.Players:Toggle()
        end
        if self.NPCs and self.NPCs.Enabled ~= tab.NPCs then
            self.NPCs:Toggle()
        end
        if self.Walls and self.Walls.Enabled ~= tab.Walls then
            self.Walls:Toggle()
        end
    end
    ...
    optionapi.Walls = components.Toggle({
        ...
        Function = function()
            local text = 'none'
            if optionapi.Walls and optionapi.Walls.Enabled then
                text = 'behind walls'
            end
    ...
    if optionapi.Walls and optionapi.Walls.Object then
...
function mainapi:CreateGUI()
    ...
    settingstitle.Position = UDim2.fromOffset(math.abs(settingstitle.Size.X.Offset), 11)
    ...
    settingsversion.Text = 'UI Library v1.0'
    ...
    function categoryapi:CreateSettingsPane(categorysettings)
        ...
        pane.BackgroundColor3 = color.Dark(uipallet.Main, 0.02)
...
function mainapi:Load(skipgui)
    ...
    -- Make clickgui visible by default
    clickgui.Visible = true
end
...
scarcitybanner.Text = 'UI Library'
...
-- Create a default Settings category that appears on the right side
...
-- Add GUI Settings to the Settings category
local guipane = SettingsCategory:CreateModule({
    Name = 'GUI Settings',
    Function = function(callback)
        -- GUI settings don't need a function
    end,
    Tooltip = 'Customize the UI appearance'
})

-- GUI Settings options
local blurToggle = guipane:CreateToggle({
    Name = 'Blur Background',
    Default = true,
    Tooltip = 'Blur the background of the GUI',
    Function = function(enabled)
        mainapi.Blur.Enabled = enabled
        mainapi:BlurCheck()
    end
})

local scaleToggle = guipane:CreateToggle({
    Name = 'Auto Rescale',
    Default = true,
    Tooltip = 'Automatically rescale the GUI based on screen resolution'
})

local scaleSlider = guipane:CreateSlider({
    Name = 'Scale',
    Min = 0.5,
    Max = 1.5,
    Default = 1,
...
local rainbowMode = guipane:CreateDropdown({
    Name = 'Rainbow Mode',
    List = {'Normal', 'Gradient', 'Retro'},
    Tooltip = 'Normal - Smooth color fade\nGradient - Gradient color fade\nRetro - Static color'
})

local rainbowSpeed = guipane:CreateSlider({
    Name = 'Rainbow Speed',
    Min = 0.1,
    Max = 5,
    Default = 1,
...
local themeColor = guipane:CreateColorSlider({
    Name = 'Theme Color',
    DefaultHue = 0.46,
    DefaultSat = 0.96,
    DefaultValue = 0.52,
    ...
    Function = function(h, s, v)
        mainapi.GUIColor = {Hue = h, Sat = s, Value = v}
        mainapi:UpdateGUI(h, s, v, true)
    end
})

local keybindButton = guipane:CreateButton({
    Name = 'Rebind GUI',
    Tooltip = 'Change the keybind to open the GUI'
})

mainapi.Scale = scaleToggle
mainapi.RainbowMode = rainbowMode
mainapi.RainbowSpeed = rainbowSpeed
mainapi.GUIColor = themeColor

-- Add the Settings category button to the main sidebar
SettingsCategory.Button:Toggle()

-- Position categories side by side (auto-sorted by the UI)
-- The categories will automatically be positioned by the UI system

-- Store the categories for external access
mainapi.Categories.Settings = SettingsCategory

gui.ResetOnSpawn = false

-- Force load with GUI visible
mainapi:Load()
